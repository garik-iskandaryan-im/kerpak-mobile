...
module.exports = {
    ...
    refund: {
        payload: ({ bankOrderId, amount, region, stripeAccount }) => {
            switch (region.paymentMethod) {
                case PROVIDERS.ID_BANK:
                    return {
        ...
        getBankOrderId: async (paymentMethod, { client, orderId, stripeAccount }) => {
            const getStripeChargeId = async () => {
                let orderData;
                try {
                    orderData = await client.getOrderStatus({
                        orderId,
                        stripeAccount
                    });
                } catch (err) {
                    log.error(err, 'order::controller::refund::getBankOrderId');
                    return { success: false, err, orderId };
                }
                if (orderData.hasError) {
                    log.error(orderData, 'order::controller::refund::getBankOrderId::hasError');
                    return { success: false, orderData, orderId };
                }
                const chargeId = orderData.data?.charges?.data?.[0]?.id;
                if (!chargeId) {
                    log.error(orderData, 'order::controller::refund::getBankOrderId::chargeId');
                    return { success: false, orderData, orderId: chargeId };
                }
                return { success: true, bankOrderId: chargeId };
            };

            switch (paymentMethod) {
                case PROVIDERS.ID_BANK:
                    return { success: true, bankOrderId: orderId };
                case PROVIDERS.STRIPE:
                case PROVIDERS.STRIPE_TEST:
                    return await getStripeChargeId();
            }
        },
    },
    register: {
