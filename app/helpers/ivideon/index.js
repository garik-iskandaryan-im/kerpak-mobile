...
class Ivideon {
    ...
    getEvent = async (videoId) => {
        const accessToken = this.accessToken;
        const url = `${ivideonUrl}/${videoId}?op=GET&access_token=${accessToken}`;
        const requestOptions = {
            json: {},
            rejectUnauthorized: false,
            responseType: 'json'
        ...
        try {
            ...
            if (err.response && err.response.data && (err.response.data.code === 'TOKEN_EXPIRED' || err.response.data.code === 'TOKEN_NOT_FOUND')) {
                this.stack.push({ url: url, requestOptions: requestOptions });
                this.getNewAccessToken();
            } else {
                log.error(err, 'ivideon::adapter:getEvent')
            }
        }
    }
