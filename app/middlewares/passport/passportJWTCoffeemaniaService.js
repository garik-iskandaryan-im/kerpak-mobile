...
module.exports.strategy = (secret) => {
    ...
    const strategy = new JwtStrategy(strategyOptions, function (request, payload, next) {
        if (payload.type) {
            ...
            switch (payload.type) {
                case CONSTANTS.JSON_TOKEN_TYPES.COFFEE_MANIA:
                    logMessage.type = 'SERVICE:CONTROLLER';
                    log.info(logMessage, 'passport::CONTROLLER::COFFEE_MANIA');
                    next();
                    break;
                default:
                    logMessage.type = 'Invalid Token';
                    log.warn(logMessage, 'passport::CONTROLLER COFFEE_MANIA invalid');
                    //invalid token
