...
const { device: deviceValidator } = require('app/schemes/integrations/coffeemania');
...
/**
 * @swagger
 * /integrations/coffeemania/devices/{id}/opendoor/:
 *   put:
 *     tags:
 *       - Coffeemania
 *     summary: Open door
 *     description: 'Запрос на разблокировку замка киоска.<br />Должен содержать ID пользователя. На данном этапе в качестве ID используется номер телефона "phone" привязанный к аккаунту. Номер телефона может быть заменен иным уникальным идентификатором.'
 *     parameters:
 *      - in: path
 *        name: id
            ...
            include: [
                {
                    ...
                    attributes: ['isCoffeemania']
                }
            ],
            raw: true
        });
        if (!kiosk['serviceProvider.isCoffeemania']) {
