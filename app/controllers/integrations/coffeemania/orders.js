...
const { orders: ordersValidator } = require('app/schemes/integrations/coffeemania');
...
/**
 * @swagger
 * /integrations/coffeemania/order:
 *   put:
 *     tags:
 *       - Coffeemania
 *     summary: Pay order
 *     description: 'Запрос на создание заказа в системе.<br/><ul><li>ID покупателя</li><li>ID киоска</li><li>ID сессии</li><li>Список штрих-кодов товаров</li></ul>'
 *     requestBody:
 *       content:
 *         application/json:
 *           schema:
...
module.exports.pay = async (req, res) => {
    try {
        ...
        if (!serviceProvider.isCoffeemania) {
        ...
        if (useBalance) {
            if (balance <= 0) {
                ...
                const order = await createOrder(serviceProvider, totalSumObj, consumerId, kiosk, null, productItemsArray, params.sessionId, true, 'coffeemania');
                if (params.sessionId) {
        ...
        const order = await createOrder(serviceProvider, totalSumObj, consumerId, kiosk, null, productItemsArray, params.sessionId, null, 'coffeemania', true);
        if (order.success === false) {
            log.error(order, 'order::pay::createOrder');
